...
export function CharacteristicsBlock({ locale, productData, section }: Props) {
	...
	return <section className='mt-8 md:mt-16'>
		<div className='gap-x-2.5 border-b border-[#E0E4E8] hidden md:flex dark:border-gray-500'>
			{ tabs.map((item, index) => {
				return <button
					key={ index }
		...
		{ tab === 'main characteristics' && <div className='flex flex-col md:flex-row my-6 md:my-4 md:gap-10'>
			<div className='flex-1'>
				{ data?.data.offer_group.width && <div className='flex md:my-4 text-sm font-medium'>
					...
					<Link href={ link(`/w-${ data?.data.offer_group.width }`) }
								className='text-[#7C7E80] dark:text-gray-50 underline max-w-max w-full hover:text-primary dark:hover:text-primary'>
			...
			<div className='flex-1'>
				...
				{ vehicleType && <div className='flex my-4 text-sm font-medium'>
					...
					<Link href={ `/vt-${ vehicleType }` } className='text-[#7C7E80] dark:text-gray-50 underline max-w-max w-full hover:text-primary dark:hover:text-primary'>
