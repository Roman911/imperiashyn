...
export function Quantity({ id = 0, price = 0, quantity, offerQuantity, onChange, setQuantity, isCart }: Props) {
	return <div className={ twMerge('quantity flex items-center', isCart && 'md:flex-col gap-6 md:items-end') }>
		<div className='flex gap-1.5'>
			<button
				onClick={() => setQuantity(id,quantity > 2 ? quantity - 2 : 2)}
				className='p-2 w-10 text-center font-bold rounded-sm text-[#575C66] border-zinc-200 bg-zinc-200 hover:bg-[#D2D3D6] transition dark:border-gray-500 dark:bg-gray-800 dark:text-[#949699]'>-
			</button>
			<input
			...
			/>
			<button
				onClick={() => setQuantity(id,quantity < offerQuantity ? quantity + 2 : offerQuantity)}
				className='p-2 w-10 text-center font-bold rounded-sm text-[#575C66] border-zinc-200 bg-zinc-200 hover:bg-[#D2D3D6] transition dark:border-gray-500 dark:bg-gray-800 dark:text-[#949699]'>+
			</button>
		</div>
